...
int create_session(VulkanSession &out) {
    ...
    // the render loop pick the right synchronization strategy up front.
    {
        ...
        LOGI("Cross-device OPAQUE_FD semaphore export: %s (compatible=0x%x exportable=0x%x importable=0x%x)",
             out.hasExportableOpaqueFdSemaphore ? "supported" : "UNSUPPORTED — will use CPU waitIdle() fallback",
             extSemProps.compatibleHandleTypes,
             extSemProps.externalSemaphoreFeatures & VK_EXTERNAL_SEMAPHORE_FEATURE_EXPORTABLE_BIT,
             extSemProps.externalSemaphoreFeatures & VK_EXTERNAL_SEMAPHORE_FEATURE_IMPORTABLE_BIT);
    }
