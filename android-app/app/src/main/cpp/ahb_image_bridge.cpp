...
// AHARDWAREBUFFER_FORMAT_B8G8R8A8_UNORM (value 5) was introduced in API 26 but
// is absent from some NDK 27 header sets when targeting android-29.  Define it
// as a numeric fallback so the translation table and import-path switch compile
// correctly on those toolchains.
...
uint32_t vk_to_ahb_format(VkFormat fmt) {
    switch (fmt) {
        ...
        case VK_FORMAT_B8G8R8A8_UNORM:           return AHARDWAREBUFFER_FORMAT_B8G8R8A8_UNORM;
...
int importAhbImage(VulkanSession &vk, AHardwareBuffer *ahb, AhbImage &out) {
    ...
    switch (desc.format) {
        ...
        case AHARDWAREBUFFER_FORMAT_R8G8B8X8_UNORM:
            out.format = VK_FORMAT_R8G8B8A8_UNORM; break;
        // BGRA is the native display format on many SoCs; map it explicitly
        // so the Vulkan layer doesn't fall back to UNDEFINED.
        case AHARDWAREBUFFER_FORMAT_B8G8R8A8_UNORM:
            out.format = VK_FORMAT_B8G8R8A8_UNORM; break;
