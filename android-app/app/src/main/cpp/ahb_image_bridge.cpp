...
int importAhbImage(VulkanSession &vk, AHardwareBuffer *ahb, AhbImage &out) {
    // ZERO-COPY: import the capture-owned AHardwareBuffer directly as Vulkan
    // external memory. No CPU lock/readback and no pixel staging buffer are
    // involved. The caller retains the AHB while this VkImage is cached.
    out = {};
