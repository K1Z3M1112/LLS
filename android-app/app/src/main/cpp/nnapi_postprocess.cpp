...
bool NnapiPostProcessor::configure(uint32_t width, uint32_t height,
    ...
    // Create the execution once and reuse it for every frame. The input/output
    // memory is stable, so only the bindings need to be refreshed per frame.
    if (ANeuralNetworksExecution_create(compilation, &execution) != kOk ||
            execution == nullptr) {
        NPU_LOGW("configure: Execution_create failed; disabling NPU stage");
        reset();
        return false;
    }
    NPU_LOGI("configure: graph compiled on NPU, burst=%p execution=%p",
             (void*)burst, (void*)execution);
    return true;
}
...
bool NnapiPostProcessor::processRgba8888(const uint8_t *src,
    ...
    // Execution is persistent; avoid per-frame NNAPI object allocation.
