...
std::vector<uint8_t> translate_dxbc_to_spirv(const std::vector<uint8_t> &bytecode) {
    if (bytecode.empty()) {
        throw std::runtime_error("translate_dxbc_to_spirv: empty shader bytecode");
    }
    ...
    if (code.size() == 0) {
        throw std::runtime_error("translate_dxbc_to_spirv: DXBC->SPIR-V compilation produced no code");
    }
    ...
    const size_t codeWordCount = code.size() / sizeof(uint32_t);
    for (size_t i = 0; i < validBindings.size(); ++i) {
        const size_t offset = validBindings[i].bindingOffset;
        if (offset >= codeWordCount) {
            throw std::runtime_error(
                "translate_dxbc_to_spirv: binding offset out of bounds "
                "(shader bytecode may be malformed or from an unsupported "
                "Lossless Scaling version)");
        }
        code.data()[offset] = static_cast<uint8_t>(i);
    }
...
int extract_dll_to_spirv(const std::string &dllPath, const std::string &cacheDir) {
    peparse::parsed_pe *dll = nullptr;
    try {
        dll = peparse::ParsePEFromFile(dllPath.c_str());
    } catch (const std::exception &e) {
        LOGE("Exception in ParsePEFromFile for %s: %s", dllPath.c_str(), e.what());
        return kErrDllUnreadable;
    } catch (...) {
        LOGE("Unknown exception in ParsePEFromFile for %s", dllPath.c_str());
        return kErrDllUnreadable;
    }
    if (!dll) {
    ...
    // RAII guard: DestructParsedPE must run no matter how we leave this
    // function (normal return or an exception thrown out of IterRsrc), or
    // the parsed_pe handle leaks.
    struct DllGuard {
        peparse::parsed_pe *handle;
        ~DllGuard() { if (handle) peparse::DestructParsedPE(handle); }
    } dllGuard{dll};

    std::unordered_map<uint32_t, std::vector<uint8_t>> blobsByResId;
    ExtractionCtx ctx{&blobsByResId};
    try {
        const int res = peparse::IterRsrc(dll, on_resource, &ctx);
        if (res != 0) {
            LOGE("IterRsrc failed for %s (rc=%d) — DLL may be corrupt", dllPath.c_str(), res);
            return kErrDllUnreadable;
        }
    } catch (const std::exception &e) {
        // pe-parse can throw on a malformed/truncated/wrong-format file (e.g.
        // an incomplete SAF copy, or the wrong file picked entirely). This
        // MUST be caught here: this function is called directly from a JNI
        // entry point with no exception handling above it, and letting a
        // C++ exception cross into the JVM/ART runtime always aborts the app.
        LOGE("Exception while parsing resources in %s: %s", dllPath.c_str(), e.what());
        return kErrDllUnreadable;
    } catch (...) {
        LOGE("Unknown exception while parsing resources in %s", dllPath.c_str());
        return kErrDllUnreadable;
    }
