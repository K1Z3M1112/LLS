...
// initContext. Duplicate-frame culling (a pixel-fingerprint filter that used
// to drop repeats before they reached framegen) has been removed, so this
// now simply tracks every frame handed to pushFrame — the device's own
// adaptive refresh rate is relied on instead of app-side filtering.
