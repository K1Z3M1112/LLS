...
struct RenderLoopConfig {
    ...
    // scaling technique this app uses. 1.0/0.0 reproduces
    // https://gamingprojects.wordpress.com/2017/12/03/reducing-pixel-blur-and-distortion/
    // exactly; see GpuPostProcessConfig for what moving each knob does.
    float gpuSharpness;
    float gpuSmoothness;
...
//
// The internal queue is a bounded FIFO (backpressure, not a drop policy):
// every frame that gets in is processed in full (real generation, never
// skipped/duplicated), but if the worker is still behind on older frames
// this call blocks briefly until a slot frees up, instead of letting an
// unbounded backlog silently grow queued latency. Safe to call from any
// thread; may block for a short time under sustained overload.
