...
// Which frame-generation backend to run. LosslessDll is the original
// reverse-engineered LSFG_3_1/LSFG_3_1P Vulkan pipeline (requires the user's
// own Lossless.dll, extracted to SPIR-V ahead of time). Ncnn is the
// alternative engine backed by the bundled frame_interpolator_v2 ncnn model
// (see ncnn_framegen.hpp) — no DLL required.
...
struct RenderLoopConfig {
    ...
    bool performance;     // selects LSFG_3_1P vs LSFG_3_1 (LosslessDll engine only)
    bool hdr;
    bool antiArtifacts;
    // Engine selection. Settings below that are specific to one engine are
    // simply ignored when the other engine is active (see initFramegen() /
    // createFramegenContext() in lsfg_render_loop.cpp for exactly which).
    FrameGenEngine engine = FrameGenEngine::LosslessDll;
    // ncnn engine only: filesystem paths to the extracted model files and
    // whether to load the fp16 or fp32 weight variant. Ignored when
    // engine == LosslessDll.
