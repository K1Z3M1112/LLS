...
bool createFramegenCompletionSemaphore(VkSemaphore &outSemaphore, int &outFd) {
    ...
    if (g.vk.fn.vkCreateSemaphore(g.vk.device, &createInfo, nullptr, &outSemaphore) != VK_SUCCESS)
        return false;
    ...
    if (g.vk.fn.vkGetSemaphoreFdKHR(g.vk.device, &fdInfo, &outFd) != VK_SUCCESS || outFd < 0) {
...
// Final display-stage pipeline only. The framegen completion semaphore is
// carried into the GPU post-process / WSI dependency chain. The returned AHB is the only object handed to the WSI
// presenter. No result is ever copied back into inSlot[] or outputs[].
bool waitFramegenSemaphoreForCpu(VkSemaphore semaphore) {
    if (semaphore == VK_NULL_HANDLE) return true;
    VkFence fence = VK_NULL_HANDLE;
    const VkFenceCreateInfo fci{ .sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO };
    if (g.vk.fn.vkCreateFence(g.vk.device, &fci, nullptr, &fence) != VK_SUCCESS)
        return false;
    const VkPipelineStageFlags stage = VK_PIPELINE_STAGE_ALL_COMMANDS_BIT;
    const VkSubmitInfo si{
        .sType = VK_STRUCTURE_TYPE_SUBMIT_INFO,
    ...
    bool done = false;
    if (submitted)
        done = g.vk.fn.vkWaitForFences(g.vk.device, 1, &fence, VK_TRUE, UINT64_MAX) == VK_SUCCESS;
    g.vk.fn.vkDestroyFence(g.vk.device, fence, nullptr);
    return done;
}
...
const AhbImage *prepareDisplayFrame(const AhbImage &src, VkSemaphore framegenDoneSemaphore = VK_NULL_HANDLE) {
    ...
    if (!g.gpuPostProcessing && framegenDoneSemaphore != VK_NULL_HANDLE) {
        // CPU/NPU post-processing needs host-visible pixels. Use the external
        // semaphore as a narrow dependency rather than draining the framegen
        // VkDevice with vkDeviceWaitIdle().
        if (!waitFramegenSemaphoreForCpu(framegenDoneSemaphore)) {
            LOGW("framegen completion semaphore CPU wait failed");
            return &src;
        }
        framegenDoneSemaphore = VK_NULL_HANDLE;
    }
...
void workerThread() {
    ...
    while (true) {
        ...
        if (runFramegen || runAi) {
            ...
            const bool useCpuWaitIdleFallback = runFramegen && !g.vk.hasExportableOpaqueFdSemaphore;
