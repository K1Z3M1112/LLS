...
bool createSwapchain() {
    ...
    const int32_t requestedMode = g.requestedPresentMode.load(std::memory_order_relaxed);
    VkPresentModeKHR presentMode = VK_PRESENT_MODE_MAILBOX_KHR;
    switch (requestedMode) {
        case 0: presentMode = VK_PRESENT_MODE_IMMEDIATE_KHR; break;
        case 2: presentMode = VK_PRESENT_MODE_FIFO_KHR; break;
        case 1:
        default: presentMode = VK_PRESENT_MODE_MAILBOX_KHR; break;
    }
...
bool blitOutputToWindow(const AhbImage &out) {
    ...
    const VkPresentModeKHR requestedVkMode =
        requestedMode == 0 ? VK_PRESENT_MODE_IMMEDIATE_KHR :
        requestedMode == 2 ? VK_PRESENT_MODE_FIFO_KHR :
...
void workerThread() {
    ...
    int64_t outputIntervalNs = 0;
    int64_t nextOutputPresentNs = 0;
    ...
    while (true) {
        ...
        // Spread generated frames across the real capture interval instead of
        // submitting the whole interpolation batch back-to-back. MAILBOX is
        // intentionally latest-image-wins; a burst of N presents can therefore
        // collapse to one visible frame per display interval even when the GPU
        // produced N distinct frames. That is the reason a 90 Hz panel could
        // appear hard-capped around 60 FPS.
        //
        // This is not VSync and it is not a fixed display FPS limiter. The
        // interval is derived from the actual capture timestamps and the
        // selected multiplier. Example: 60 Hz input at 2x => ~120 Hz output
        // cadence; a 90 Hz display can consume up to its own 90 Hz capacity.
        if (havePrevCaptureTimestamp &&
            pendingFrame.captureTimestampNs > prevCaptureTimestampNs) {
            const int64_t captureDeltaNs =
                pendingFrame.captureTimestampNs - prevCaptureTimestampNs;
            const int64_t totalOutputFrames =
                static_cast<int64_t>(g.outputs.size()) + 1; // generated + real
            if (totalOutputFrames > 0) {
                outputIntervalNs = captureDeltaNs / totalOutputFrames;
                // Numerical floor only: prevent a zero-duration busy burst when
                // the capture timestamp source is coarse or temporarily invalid.
                if (outputIntervalNs < 1'000'000) outputIntervalNs = 1'000'000;
            }
        }
        ...
        if (runFramegen || runAi) {
            ...
            auto paceOutput = [&]() {
                if (outputIntervalNs <= 0) return;

                const auto now = State::Clock::now();
                const int64_t nowNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
                    now.time_since_epoch()).count();

                // If the pipeline is already more than two output intervals late,
                // restart the cadence from "now" rather than building unbounded
                // presentation latency. No generated frame is auto-disabled.
                if (nextOutputPresentNs == 0 ||
                    nowNs > nextOutputPresentNs + outputIntervalNs * 2) {
                    nextOutputPresentNs = nowNs;
                }

                if (nowNs < nextOutputPresentNs) {
                    std::this_thread::sleep_until(
                        State::Clock::time_point(
                            std::chrono::nanoseconds(nextOutputPresentNs)));
                }

                const auto afterSleep = State::Clock::now();
                const int64_t afterSleepNs =
                    std::chrono::duration_cast<std::chrono::nanoseconds>(
                        afterSleep.time_since_epoch()).count();
                nextOutputPresentNs = std::max(nextOutputPresentNs, afterSleepNs) + outputIntervalNs;
            };
            auto timedBlit = [&blitWorkNsThisFrame, &pendingFrame, &prof, &paceOutput](const AhbImage &out) {
                paceOutput();
...
void setPresentMode(int32_t mode) {
    // Three independent modes. Invalid values are rejected instead of being
    // converted into another mode, so there is never an implicit fallback.
    if (mode < 0 || mode > 2) {
        LOGW("Ignoring invalid Vulkan present mode=%d (no fallback)", mode);
        return;
    }

    std::lock_guard<std::mutex> lock(g.mu);
    const int32_t oldMode = g.requestedPresentMode.load(std::memory_order_relaxed);
    if (oldMode == mode) return;

    // First stop accepting work for the old presentation mode. destroySwapchain()
    // performs vkDeviceWaitIdle(), which drains every in-flight acquire/present/
    // blit submission before destroying the old mode's semaphores and images.
    g.requestedPresentMode.store(mode, std::memory_order_relaxed);
    if (g.swap.swapchain != VK_NULL_HANDLE || g.swap.surface != VK_NULL_HANDLE) {
        destroySwapchain();
    }

    LOGI("Vulkan present mode switched atomically: %s -> %s; old mode queue cleaned",
         oldMode == 0 ? "IMMEDIATE" : (oldMode == 1 ? "MAILBOX" : "FIFO"),
         mode == 0 ? "IMMEDIATE" : (mode == 1 ? "MAILBOX" : "FIFO"));
}
