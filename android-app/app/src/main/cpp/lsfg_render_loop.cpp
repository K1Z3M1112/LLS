...
struct State {
    ...
    // Optional display-stage scaling. GPU-only: a single Vulkan compute pass
    // running the sharp-bilinear-simple technique (see gpu_postprocess.comp).
    // This buffer is never fed back into frame generation; it exists only
    // between the completed frame and blitOutputToSwapchain().
    bool gpuPostProcessing = false;
    float gpuUpscaleFactor = 1.0f;
    float gpuSharpness = 1.0f;
    float gpuSmoothness = 0.0f;
    ...
    // Bounded FIFO for captured frames. Every frame that is admitted into
    // the queue is retained until the worker processes it in full — there is
    // still no latest-frame replacement, overflow drop, filtering, or
    // frame-rate gate. What's bounded is *admission*: pushFrame() blocks
    // (backpressure) once kMaxPendingFrames are already queued, instead of
    // growing the deque without limit.
    //
    // Why: with an unbounded queue, if the capture rate ever outruns the
    // generation throughput (GPU/AI busy > capture interval), the backlog
    // grows forever and so does the latency of every frame sitting behind
    // it — the exact opposite of "lowest possible latency", even though no
    // frame is ever skipped or duplicated. Bounding admission caps the
    // worst case: at most kMaxPendingFrames frame-times of queued latency,
    // ever. Every admitted frame still goes through the full real
    // generation pipeline (import → copy → presentContext → waitIdle →
    // blit every generated output + the real frame) — nothing is skipped,
    // downsampled, or replaced by a stale/duplicate frame. If the device is
    // consistently too slow to keep up (see the `queue=` figure in the
    // periodic frame-profile log), the fix is to lower multiplier/
    // flowScale/resolution, not to silently drop frames here.
    ...
    std::condition_variable spaceCv;   // signaled when a slot frees up (producer wakes)
...
bool blitOutputToSwapchain(const AhbImage &src, VkSemaphore framegenDoneSemaphore = VK_NULL_HANDLE) {
    ...
    uint32_t imageIdx = 0;
    // Strictly non-blocking acquire: never wait for display/refresh cadence.
    // MAILBOX is allowed to keep/reuse the newest submitted image; if no image
    // is immediately available, drop this submission rather than add latency.
    const VkResult ar = g.vk.fn.vkAcquireNextImageKHR(
        g.vk.device, g.swap.swapchain, 0, acquireSem, VK_NULL_HANDLE, &imageIdx);
    if (ar == VK_ERROR_OUT_OF_DATE_KHR) {
        g.swap.outOfDate = true;
        return false;
    }
    if (ar == VK_NOT_READY || ar == VK_TIMEOUT) {
        // No waiting for WSI. The next generated frame gets the next attempt.
        return false;
    }
...
void drainFramegenCompletionSemaphore(VkSemaphore semaphore) {
    ...
    g.framegenCompletionTickets.push_back({semaphore, fence});
}
...
// Display-stage post-processing is GPU-only now: a single Vulkan compute
// pass running the sharp-bilinear-simple technique (see gpu_postprocess.comp). The
// old CPU/NPU readback-process-writeback path is gone entirely, so there is
// no host round-trip and no extra semaphore wait here.
const AhbImage *prepareDisplayFrame(const AhbImage &src, VkSemaphore framegenDoneSemaphore = VK_NULL_HANDLE) {
    if (!g.gpuPostProcessing) return &src;
    ...
    const GpuPostProcessConfig cfg{
        ...
        .smoothness = std::clamp(g.gpuSmoothness, 0.0f, 1.0f),
    };
    if (g.gpuPost.process(g.vk, src, g.postImage, cfg, framegenDoneSemaphore)) {
        return &g.postImage;
    }
...
void workerThread() {
    ...
    while (true) {
        ...
        // A slot in the bounded queue just freed up — wake any pushFrame()
        // call blocked waiting for room. Outside the lock is fine: spaceCv's
        // predicate re-checks pendingFrames.size() itself under the lock.
        g.spaceCv.notify_one();
        ...
        if (runFramegen || runAi) {
            ...
            if (runFramegen && !useCpuWaitIdleFallback) {
                ...
                if (!syncReady) {
                    ...
                    // session-level capability mismatch instead of retrying
                    // the same operation on every frame.
                    g.crossDeviceSyncDisabled = true;
                    useCpuWaitIdleFallback = true;
                    LOGW("External semaphore FD creation rejected by driver; disabling cross-device FD sync for this session");
                }
            }
            if (runFramegen) {
            ...
            if (useCpuWaitIdleFallback) {
                // No exportable semaphore was signaled — block on framegen's
                // own device until this present's GPU work is done, so the
                // outputs are safe to read below without any wait semaphore.
                try {
                    if (g.performanceMode)
                        LSFG_3_1P::waitIdle();
                    else
            ...
            auto timedBlit = [&blitWorkNsThisFrame, &pendingFrame, &prof](const AhbImage &out, VkSemaphore framegenDone = VK_NULL_HANDLE) {
                ...
                const AhbImage *displayOut = prepareDisplayFrame(out, framegenDone);
                const bool posted = blitOutputToWindow(*displayOut, framegenDone);
                ...
                if (posted && g.gpuPostProcessing && g.vk.fn.vkQueueWaitIdle != nullptr) {
            ...
            // GAME-STYLE UNCAP: once the GPU/AI work for this capture is
            // complete, publish every generated frame and the real frame
            // immediately. There is deliberately NO software frame pacing,
            // sleep_until(), Choreographer deadline, or output FPS cap here.
            // The selected Vulkan WSI mode (MAILBOX/FIFO/IMMEDIATE) owns the
            // presentation semantics; this worker never waits for display Hz.
...
int initRenderLoop(const char *cacheDir, const RenderLoopConfig &cfg) {
    ...
    g.gpuSmoothness = std::clamp(cfg.gpuSmoothness, 0.0f, 1.0f);
...
void pushFrame(AHardwareBuffer *ahb, int64_t timestampNs) {
    ...
    AHardwareBuffer_acquire(ahb);
    {
        ...
        // Backpressure instead of unbounded growth: block here until the
        // worker has drained the queue below the cap. This is still a pure
        // FIFO — nothing already queued is ever replaced, filtered, or
        // discarded, and this call itself never drops `ahb`. It just refuses
        // to admit a NEW frame past the cap until an old one has fully
        // finished (import → framegen → blit), which bounds queued latency
        // instead of letting it grow without limit.
        g.spaceCv.wait(lock, [] {
            return g.stopRequested ||
                   g.pendingFrames.size() < State::kMaxPendingFrames;
        });
        if (g.stopRequested) {
            AHardwareBuffer_release(ahb);
            return;
...
void shutdownRenderLoop() {
    ...
    g.spaceCv.notify_all(); // release any pushFrame() blocked on backpressure
