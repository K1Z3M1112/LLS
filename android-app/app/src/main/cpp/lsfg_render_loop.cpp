...
bool blitOutputToSwapchain(const AhbImage &src) {
    ...
    // No present-mode fallback: IMMEDIATE remains IMMEDIATE. Serialize only
    // the WSI queue on drivers that expose IMMEDIATE but cannot safely retire
    // multiple presents concurrently. This does not add software vsync.
    if (g.swap.presentMode == VK_PRESENT_MODE_IMMEDIATE_KHR &&
        g.vk.fn.vkQueueWaitIdle != nullptr) {
        const VkResult idle = g.vk.fn.vkQueueWaitIdle(g.vk.computeQueue);
        if (idle != VK_SUCCESS) {
            LOGW("IMMEDIATE vkQueueWaitIdle returned %d", (int)idle);
            return false;
        }
    }
    return true;
...
void setPresentMode(int32_t mode) {
    const int32_t clamped = (mode == 0 || mode == 2) ? mode : 1;
    g.requestedPresentMode.store(clamped, std::memory_order_relaxed);
    LOGI("Vulkan present mode selected: %s",
         clamped == 0 ? "IMMEDIATE" : (clamped == 2 ? "FIFO" : "MAILBOX"));
}
