...
struct State {
    ...
    // Engine selection (see FrameGenEngine in lsfg_render_loop.hpp). Every
    // call site that talks to LSFG_3_1(P):: is mirrored with a NcnnFG:: call
    // when engine == Ncnn — see initFramegen()/createFramegenContext() below
    // and the presentContext/waitIdle/deleteContext/finalize call sites in
    // the worker thread and shutdownRenderLoop().
    FrameGenEngine engine = FrameGenEngine::LosslessDll;
    std::string ncnnParamPath;
    std::string ncnnBinPath;
    bool ncnnUseVulkanCompute = true;
...
bool initFramegen(const char *cacheDir) {
    if (g.engine == FrameGenEngine::Ncnn) {
        // No Lossless.dll shader cache involved at all — the ncnn engine
        // loads its own model files directly.
        if (g.ncnnParamPath.empty() || g.ncnnBinPath.empty()) {
            LOGE("Ncnn engine selected but ncnnParamPath/ncnnBinPath are empty");
            return false;
        }
        try {
            NcnnFG::initialize(g.hdr, g.flowScale,
                static_cast<uint64_t>(g.multiplier),
                g.ncnnParamPath, g.ncnnBinPath, g.ncnnUseVulkanCompute);
            return true;
        } catch (const std::exception &e) {
            LOGE("NcnnFG::initialize threw: %s", e.what());
            return false;
        } catch (...) {
            LOGE("NcnnFG::initialize threw unknown exception");
            return false;
        }
    }
...
bool createFramegenContext() {
    ...
    try {
        if (g.engine == FrameGenEngine::Ncnn) {
            g.framegenCtxId = NcnnFG::createContextFromAHB(
...
void workerThread() {
    ...
    while (true) {
        ...
        if (runFramegen) {
            ...
            try {
                if (g.engine == FrameGenEngine::Ncnn)
                    NcnnFG::presentContext(g.framegenCtxId, /*inSem*/ -1, outSems);
                else if (g.performanceMode)
            ...
            try {
                if (g.engine == FrameGenEngine::Ncnn) NcnnFG::waitIdle();
                else if (g.performanceMode)           LSFG_3_1P::waitIdle();
...
int initRenderLoop(const char *cacheDir, const RenderLoopConfig &cfg) {
    ...
    g.hdr = cfg.hdr;
    g.engine = cfg.engine;
    g.ncnnParamPath = cfg.ncnnParamPath;
    g.ncnnBinPath = cfg.ncnnBinPath;
    g.ncnnUseVulkanCompute = cfg.ncnnUseVulkanCompute;
...
void shutdownRenderLoop() {
...

    {
        ...
        if (g.framegenCtxId >= 0) {
            try {
                if (g.engine == FrameGenEngine::Ncnn) NcnnFG::deleteContext(g.framegenCtxId);
                else if (g.performanceMode)           LSFG_3_1P::deleteContext(g.framegenCtxId);
        ...
        if (g.framegenInitOk) {
            try {
                if (g.engine == FrameGenEngine::Ncnn) NcnnFG::finalize();
                else if (g.performanceMode)           LSFG_3_1P::finalize();
