...
struct State {
    ...
    // Counts every capture frame pushed into the pipeline. Used by the HUD's
    // "real fps" counter. Previously this only counted frames that survived
    // a pixel-duplicate filter (MediaProjection can deliver at the display
    // refresh rate regardless of the target app's render rate), but that
    // filtering has been removed — the device's own adaptive-refresh-rate
    // handling is relied on instead, so every pushed frame counts.
...
// Duplicate-capture fingerprinting/culling was removed here: it used to
// hash each frame and drop pixel-identical repeats before they entered the
// framegen pipeline. The device's own adaptive-refresh-rate handling is
// relied on instead, so every captured frame is now pushed through
// unconditionally (see pushFrame below).
...
void pushFrame(AHardwareBuffer *ahb, int64_t timestampNs) {
    ...
    // Duplicate-frame culling removed: every captured frame is pushed into
    // the pipeline regardless of whether its pixel content matches the
    // previous capture. The device's adaptive refresh rate is relied on to
    // avoid wasted work instead of an app-side duplicate filter.
