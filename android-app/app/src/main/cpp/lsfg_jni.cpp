...
Java_com_lsfg_android_session_NativeBridge_initContext(
        ...
        jint queueDepth,
        jint engine, jstring ncnnParamPath, jstring ncnnBinPath,
        jboolean ncnnUseVulkanCompute) {
    const std::string cache = jstring_to_std(env, cacheDir);
    if (cache.empty() || width <= 0 || height <= 0) {
        return lsfg_android::kErrDllUnreadable;
    }
    const auto selectedEngine = (engine == 1)
        ? lsfg_android::FrameGenEngine::Ncnn
        : lsfg_android::FrameGenEngine::LosslessDll;

    const std::string ncnnParam = jstring_to_std(env, ncnnParamPath);
    const std::string ncnnBin = jstring_to_std(env, ncnnBinPath);

    if (selectedEngine == lsfg_android::FrameGenEngine::Ncnn) {
        // The ncnn engine ships its own model files (extracted from app
        // assets by the Kotlin side before calling initContext) and never
        // touches the Lossless.dll SPIR-V cache, so the probe below doesn't
        // apply — skip straight to validating the model paths instead.
        if (ncnnParam.empty() || ncnnBin.empty()) {
    ...
    const lsfg_android::RenderLoopConfig cfg{
        ...
        .ncnnBinPath = ncnnBin,
        .ncnnUseVulkanCompute = ncnnUseVulkanCompute == JNI_TRUE,
    };
