...
Java_com_lsfg_android_session_NativeBridge_extractShaders(
    ...
    try {
        ...
        // This function is called directly from Kotlin across the JNI
        // boundary. A C++ exception must never be allowed to propagate into
        // the JVM/ART runtime — doing so aborts the entire app rather than
        // just failing this one operation, which is exactly the crash the
        // "Extract Shaders" button was hitting.
        __android_log_print(ANDROID_LOG_ERROR, "lsfg-extract",
            "Uncaught exception in extractShaders: %s", e.what());
        return lsfg_android::kErrInternal;
    } catch (...) {
        __android_log_print(ANDROID_LOG_ERROR, "lsfg-extract",
            "Unknown uncaught exception in extractShaders");
        return lsfg_android::kErrInternal;
    }
}
