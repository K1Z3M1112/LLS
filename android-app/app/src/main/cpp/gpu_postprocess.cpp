...
bool GpuPostProcessor::process(VulkanSession &vk,
    ...
    pending.push_back(PendingSubmit{cb, srcView, dstView, fence});
    return true;
}
...
void GpuPostProcessor::reapPending(VulkanSession &vk) {
    ...
    for (auto &p : pending) {
        ...
        if (p.srcView != VK_NULL_HANDLE) vk.fn.vkDestroyImageView(vk.device, p.srcView, nullptr);
        if (p.dstView != VK_NULL_HANDLE) vk.fn.vkDestroyImageView(vk.device, p.dstView, nullptr);
