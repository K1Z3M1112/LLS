...
// Fast inline rounding — replaces std::lround() (a C-library call) with a
// branchless idiom that compiles to a single VCVTR/VCVT instruction on ARM.
// Equivalent to std::lround for all finite float inputs (round-half-away).
inline int fastRound(float v) {
...
// Contrast around the midpoint, then optional per-channel tint.
// Uses fastRound instead of std::lround — same result, no library call.
...
void CpuPostProcessor::rebuildLut() {
    ...
    switch (cfg.preset) {
        ...
        case CpuPreset::CINEMATIC:
            // Luma-dependent path handled per-pixel; LUT stays identity.
            break;
    }
...
bool CpuPostProcessor::configure(uint32_t width, uint32_t height,
    ...
    // FIX: capture the previous vignette strength BEFORE overwriting cfg.
    // Comparing cfg.vignette with config.vignette after "cfg = config" always
    // yields zero — they are the same object at that point.
    const float prevVignette = cfg.vignette;
    cfg = config;
    rebuildLut();

    const bool vignetteChanged = std::fabs(cfg.vignette - prevVignette) > 0.001f;
    if (width != lastW || height != lastH || vignetteMask.empty() || vignetteChanged) {
        lastW = width;
        lastH = height;
        ...
        if (cfg.vignette > 0.0001f && width > 0 && height > 0) {
            ...
            for (uint32_t y = 0; y < height; ++y) {
                ...
                for (uint32_t x = 0; x < width; ++x) {
                    ...
                    const float darken = 1.0f - strength *
                                         (1.0f - std::cos(kPi * 0.5f * t));
                    vignetteMask[y * width + x] =
                        static_cast<uint16_t>(fastRound(darken * 256.0f));
                }
            }
        }
    }
...
bool CpuPostProcessor::process(const uint8_t *src, uint32_t srcStride,
    ...
    // Saturation centred at 0.5 → identity; each 0.1 step = ±0.06.
    ...
    // Hoist the fixed-point conversion out of the inner loop.
    const int   sharpFixed    = static_cast<int>(sharpAmount * 256.0f + 0.5f);

    // Pre-compute cinematic scale factors (loop-invariant).
    const float cinematicS    = clamp01(cfg.strength);
    const float cineHighScale = 32.0f * cinematicS;   // R highlight boost
    const float cineShadScale = 32.0f * cinematicS;   // B shadow push
    const float cineGScale    =  6.0f * cinematicS;   // G mild lift
    ...
    for (uint32_t y = 0; y < h; ++y) {
        ...
        // Optional 3×3 blur for GAMER_SHARP unsharp-mask.
        if (doSharp) {
            ...
            for (uint32_t x = 0; x < w; ++x) {
                ...
                for (int ch = 0; ch < 3; ++ch) {
                    ...
                    blurRow[x*4+ch] = static_cast<uint8_t>(sum >> 4);
                }
        ...
        for (uint32_t x = 0; x < w; ++x) {
            ...
            if (doSharp) {
                ...
                // sharpFixed = sharpAmount * 256; avoids per-pixel float mul.
                r = r + (((r - br) * sharpFixed) >> 8);
                g = g + (((g - bg) * sharpFixed) >> 8);
                b = b + (((b - bb) * sharpFixed) >> 8);
            }
            ...
            if (doCinematic) {
                // Teal-and-orange: boost red in highlights, push blue in shadows.
                const int yLuma = (77 * r + 150 * g + 29 * b) >> 8;
                const float t = yLuma * (1.0f / 255.0f);
                // fastRound replaces std::lround — same semantics, no library call.
                r = clamp_u8(r + fastRound((t - 0.5f) * cineHighScale));
                b = clamp_u8(b + fastRound(((1.0f - t) - 0.5f) * cineShadScale));
                g = clamp_u8(g + fastRound((t - 0.5f) * cineGScale));
            }
            ...
            if (doSat || doVibr) {
                ...
                if (doVibr) {
                    ...
                    const float localSat = mx > 0
                        ? static_cast<float>(mx - mn) / static_cast<float>(mx)
                        : 0.0f;
                    ss += vibr * 0.30f * (1.0f - localSat);
                }
                // fastRound for saturation scaling — avoids three lround calls.
                r = clamp_u8(yLuma + fastRound(static_cast<float>(r - yLuma) * ss));
                g = clamp_u8(yLuma + fastRound(static_cast<float>(g - yLuma) * ss));
                b = clamp_u8(yLuma + fastRound(static_cast<float>(b - yLuma) * ss));
            }
