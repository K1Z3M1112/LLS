...
import com.firstt175.deepdrop.session.diagnostics.CrashReporter
...
class SettingsDrawerOverlay(
     ...
     * taps START SESSION. LsfgForegroundService.handleStart() then builds a
    ...
    private fun buildPanel(): View {
        ...
        frameGenSection.addView(switchRow(
            ...
            queueNativeAction("waitForBusyGeneration") {
                runCatching { NativeBridge.setWaitForBusyGeneration(it) }
                    .onFailure { e -> LsfgLog.w(TAG, "setWaitForBusyGeneration failed", e) }
            }
        })
        ...
        frameGenSection.addView(switchRow(
            ...
            queueNativeAction("allowGenerationWhenBusy") {
                runCatching { NativeBridge.setAllowGenerationWhenBusy(it) }
                    .onFailure { e -> LsfgLog.w(TAG, "setAllowGenerationWhenBusy failed", e) }
            }
        })
        ...
        frameGenSection.addView(switchRow(
            ...
            queueNativeAction("losslessQueue") {
                runCatching { NativeBridge.setLosslessQueue(it) }
                    .onFailure { e -> LsfgLog.w(TAG, "setLosslessQueue failed", e) }
            }
        })
        ...
        val resumeFramegenBtn = Button(ctx).apply {
            ...
            setOnClickListener {
                val wasLatched = runCatching { NativeBridge.resumeFramegenAfterAutoDisable() }
                    .onFailure { e -> LsfgLog.w(TAG, "resumeFramegenAfterAutoDisable failed", e) }
                    .getOrDefault(false)
        ...
        val iePrefs = LsfgPreferences(ctx)
        ...
        val upPrefs = LsfgPreferences(ctx)
        ...
        fun paintUpscaleButtons() {
            upscaleButtons.forEachIndexed { index, b ->
                ...
                b.setTextColor(if (selected) COLOR_PANEL_BG else COLOR_ON_SURFACE)
        ...
        listOf("Nearest", "Bilinear").forEachIndexed { index, label ->
            val b = Button(ctx).apply {
                ...
                setOnClickListener {
                    upFilter = index
                    upPrefs.setUpscaleFilter(if (index == 0) com.firstt175.deepdrop.prefs.UpscaleFilter.NEAREST else com.firstt175.deepdrop.prefs.UpscaleFilter.BILINEAR)
                    queueNativeAction("upscale") {
                        NativeBridge.setUpscaleEnabled(upEnabled)
                        NativeBridge.setUpscaleFilter(upFilter)
                    }
        ...
        val pmPrefs = LsfgPreferences(ctx)
        val presentModeValues = listOf(
            com.firstt175.deepdrop.prefs.PresentMode.IMMEDIATE,
            com.firstt175.deepdrop.prefs.PresentMode.MAILBOX,
            com.firstt175.deepdrop.prefs.PresentMode.FIFO,
        )
        ...
        // ---- HUD (single switch: fps, frame graph, CPU/GPU/RAM) ---------------------
        ...
        hudSection.addView(switchRow(
            ...
            initial = initialHudPositionUnlocked,
        ) {
        ...
        tabLabels.forEachIndexed { index, label ->
            val button = Button(ctx).apply {
                ...
                setTextColor(if (index == 0) COLOR_PANEL_BG else COLOR_ON_SURFACE)
        ...
        val btn = Button(ctx).apply {
            ...
            setOnClickListener {
                ...
                actions.forEach { action ->
                    runCatching { action() }
                        .onFailure { e -> LsfgLog.w(TAG, "queued Apply action failed", e) }
                }
            }
        }
