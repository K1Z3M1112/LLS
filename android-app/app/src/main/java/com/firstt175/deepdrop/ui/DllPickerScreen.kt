...
import com.firstt175.deepdrop.session.CustomModelImporter
...
fun DllPickerScreen(nav: NavHostController) {
    ...
    var customPending by remember { mutableStateOf<Pair<Uri, String>?>(null) }
    ...
    val myModelPicker = rememberLauncherForActivityResult(
        ActivityResultContracts.OpenDocument(),
    ) { uri: Uri? ->
        if (uri == null) return@rememberLauncherForActivityResult
        val resolver = ctx.contentResolver
        ...
        val name = resolver.query(uri, arrayOf(OpenableColumns.DISPLAY_NAME), null, null, null)?.use {
            if (it.moveToFirst()) it.getString(0) else null
        } ?: uri.lastPathSegment ?: "model.zip"
        customPending = uri to name
        customModelMessage = "Importing $name…"
    }

    LaunchedEffect(customPending) {
        val pending = customPending ?: return@LaunchedEffect
        customPending = null
        runCatching {
            withContext(Dispatchers.IO) { CustomModelImporter.import(ctx, pending.first, pending.second) }
        }.onSuccess { result ->
            prefs.setActiveModel(result.dir.absolutePath, result.label, result.engine)
            prefs.setFramegenBackend(FramegenBackend.NCNN_AI)
            prefs.setAiEngine(if (result.engine == 1) AiEngine.IFRNET else AiEngine.RIFE)
            refreshConfigState(prefs)
            customModelMessage = "My Model ready: ${result.label}"
            testLoadNonce++
        }.onFailure { customModelMessage = "Import failed: ${it.message}" }
    }

    LaunchedEffect(pendingAssetModel) {
        val model = pendingAssetModel ?: return@LaunchedEffect
        pendingAssetModel = null
        runCatching { withContext(Dispatchers.IO) { AssetModelScanner.extract(ctx, model) } }
            .onSuccess { dir ->
            ...
            .onFailure { customModelMessage = "Asset model failed: ${it.message}" }
    }
    ...
    Column(
            ...
            assetModels.forEach { model ->
                ...
                Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
                    Column(modifier = Modifier.weight(1f)) {
                        ...
                        Text(if (model.engine == 1) "IFRNet" else "RIFE", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                    Button(
                        onClick = { pendingAssetModel = model },
                        enabled = !active,
                        content = { Text(if (active) "USED" else "USE") },
                    )
                }
            }
            Spacer(Modifier.height(8.dp))
            Text(
                text = "Every model folder under assets/models/ is discovered automatically. USE makes that exact asset the active NCNN model.",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
        }
        ...
        LsfgCard {
            ...
            Spacer(Modifier.height(8.dp))
            Text(
                text = "Upload a trained NCNN model exported from the notebook (.zip, .param/.bin). After import it becomes the active model.",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            Spacer(Modifier.height(10.dp))
            LsfgSecondaryButton(
                text = "UPLOAD MODEL",
                onClick = { myModelPicker.launch(arrayOf("application/zip", "application/octet-stream", "*/*")) },
                leadingIcon = Icons.Filled.FileOpen,
                modifier = Modifier.fillMaxWidth(),
            )
            if (state.activeModelName != null) {
                Spacer(Modifier.height(8.dp))
                Text("ACTIVE: ${state.activeModelName}", style = MaterialTheme.typography.bodySmall, color = LsfgStatusGood)
            }
