...
class OverlayManager(private val ctx: Context) {
    ...
    fun show() {
        ...
        tex.surfaceTextureListener = object : TextureView.SurfaceTextureListener {
            ...
            override fun onSurfaceTextureSizeChanged(st: SurfaceTexture, width: Int, height: Int) {
                ...
                if (s != null) applyFixedOutputFrameRate(s)
                syncOverlayGeometry()
                if (s != null) {
    ...
    /**
     * Pins the output Surface to the display's highest supported refresh rate.
     * This deliberately does not sleep/throttle the render loop and does not
     * implement VSync; it only tells SurfaceFlinger that this continuously-fed
     * surface is a fixed-rate source. The request is repeated only on surface
     * creation/resize, never from the per-frame path.
     */
    private fun applyFixedOutputFrameRate(surface: Surface) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R || !surface.isValid) return
        val wm = hostWindowManager ?: return
        runCatching {
            @Suppress("DEPRECATION")
            val modes = wm.defaultDisplay.supportedModes
            val best = modes.maxByOrNull { it.refreshRate } ?: return@runCatching
            val hz = best.refreshRate
            if (hz > 0f) {
                ...
                Log.i(TAG, "Output Surface fixed-rate request=${hz}Hz mode=${best.modeId}")
            }
        }.onFailure {
            Log.d(TAG, "Surface fixed-rate request unavailable: ${it.message}")
        }
    }
