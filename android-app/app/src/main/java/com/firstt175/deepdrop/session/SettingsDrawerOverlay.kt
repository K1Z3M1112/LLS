...
class SettingsDrawerOverlay(
    ...
    private fun buildPanel(): View {
        ...
        // ---- Post-processing ----------------------------------------------------------
        // Display-stage processing runs after frame generation. These controls mirror
        // ParamsScreen so the same pipeline can be configured without leaving the overlay.
        val postProcessingSection = collapsibleSection(panel, "POST-PROCESSING")
        postProcessingSection.addView(switchRow(
        ...
        if (initial.gpuPostProcessing) {
            ...
            postProcessingSection.addView(
                sliderRow(
                    labelText = "GPU upscale factor",
                    valueView = gpuUpscaleValue,
                ),
            )
            postProcessingSection.addView(SeekBar(ctx).apply {
                max = 1
                progress = ((initial.gpuUpscaleFactor - 1.0f) * 1f).toInt().coerceIn(0, 1)
                progressDrawable = buildSeekTrack()
                thumb = buildSeekThumb()
                splitTrack = false
                wireLiveDrag(
                    label = "gpuUpscaleFactor",
                    toValue = { p -> if (p <= 0) 1.0f else 2.0f },
        ...
        val npuPostAvailable = runCatching {
            NativeBridge.isNpuPostProcessAvailable()
        }.getOrDefault(false)
        if (npuPostAvailable) {
            postProcessingSection.addView(divider())
            postProcessingSection.addView(switchRow(
        ...
        val presentItems = listOf(
            ...
            com.firstt175.deepdrop.prefs.PresentMode.IMMEDIATE to "IMMEDIATE",
            com.firstt175.deepdrop.prefs.PresentMode.FIFO to "FIFO",
        )
