    ...
    private fun runCaptureLoop(
        ...
        while (running.get()) {
            ...
            if (hb != null) {
                ...
                try {
                    // Capture is intentionally dumb: forward every acquired frame.
                    // Queueing, pacing, dropping and latency policy belong to RenderLoop.
                    // ZERO-COPY: pass the captured HardwareBuffer handle directly.
                    // No Bitmap/ByteBuffer mapping, CPU readback, or pixel memcpy occurs here.
                    // The client-side NativeBridge acquires the same native AHardwareBuffer
                    // and imports it directly into Vulkan external memory.
                    callback.onFrame(hb, timestampNs, nextFrameId.incrementAndGet())
                } catch (t: Throwable) {
