...
object NativeBridge {
    ...
    /** Attaches an optional hardware-encoder surface. Frames are teed after the display post. */
    external fun setRecordingSurface(surface: Surface?, w: Int, h: Int)
    ...
    /** Writes a vkBasalt-compatible configuration generated by the settings UI. */
    external fun writeVkBasaltConfig(
        configPath: String,
        enabled: Boolean,
        effects: Array<String>,
        reshadeTexturePath: String,
        reshadeIncludePath: String,
        depthCapture: Boolean,
        casSharpness: Float,
        dlsSharpness: Float,
        dlsDenoise: Float,
        fxaaQualitySubpix: Float,
        fxaaQualityEdgeThreshold: Float,
        fxaaQualityEdgeThresholdMin: Float,
        smaaThreshold: Float,
        smaaMaxSearchSteps: Int,
        smaaMaxSearchStepsDiag: Int,
        smaaCornerRounding: Int,
        lutFile: String,
    ): Boolean
    ...
    /** Automatically skip GPU frame generation once a REAL frame reaches the configured age. 0 disables. */
    external fun setBypassGenDeadlineMs(deadlineMs: Int)

    /** Keep BypassGen active for this many milliseconds before allowing generation to resume. */
    external fun setBypassGenResumeDelayMs(delayMs: Int)
