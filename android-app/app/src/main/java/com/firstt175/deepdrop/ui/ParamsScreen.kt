...
fun ParamsFrameGenPacingScreen(nav: NavHostController) {
    ...
    Column(
        ...
        // ---- Post-processing -------------------------------------------------------
        // Optional and off by default, so it starts collapsed for a first-time
        // user — but stays open automatically once the toggle is on, so an
        // active setting is never hidden behind a tap.
        CollapsibleSection(
            title = "Post-processing",
            subtitle = if (state.gpuPostProcessing) {
                "On"
            } else {
                "Off — optional, tap to configure"
            },
            startExpanded = state.gpuPostProcessing,
        ) {
            Text(
                text = "Optional display-stage scaling. It runs after frame generation and does not replace or alter the frame-generation loop. GPU-only: a single lightweight pass running the sharp-bilinear-simple technique (integer pre-scale + linear blend at pixel edges), no CPU or NPU stage.",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            Spacer(Modifier.height(8.dp))
            ...
            ToggleRow(
                ...
                title = "GPU upscale (Sharp Bilinear)",
            ...
            if (state.gpuPostProcessing) {
                ...
                ValueSlider(
                    title = "Sharpness",
                    valueDisplay = "%.0f%%".format(state.gpuSharpness * 100f),
                    description = "How much of each source pixel stays flat/point-filtered. 100% matches the technique exactly (pixel-perfect, no wobble); lower values fade toward plain bilinear (softer).",
                ...
                ValueSlider(
                    title = "Smoothness",
                    valueDisplay = "%.0f%%".format(state.gpuSmoothness * 100f),
                    description = "How gently the blend ramps at pixel edges. 0% is a straight linear blend (matches the technique exactly); higher values ease the curve to soften banding at non-integer scale factors.",
                    value = state.gpuSmoothness,
                    range = 0.0f..1.0f,
                    steps = 19,
                    leadingIcon = Icons.Filled.Tune,
                    onValueChange = {
                        prefs.setGpuSmoothness(it)
                        refreshConfigState(prefs)
                    },
                )
            }
