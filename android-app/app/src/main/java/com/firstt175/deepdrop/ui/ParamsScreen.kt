...
fun ParamsFrameGenPacingScreen(nav: NavHostController) {
    ...
    Column(
        ...
        // user — but stays open automatically once any of its toggles are on,
        // so an active setting is never hidden behind a tap.
        CollapsibleSection(
            title = "Post-processing",
            subtitle = if (state.gpuPostProcessing || state.npuPostProcessing || state.cpuPostProcessing) {
                "On"
            } else {
                "Off — optional, tap to configure"
            },
            startExpanded = state.gpuPostProcessing || state.npuPostProcessing || state.cpuPostProcessing,
        ) {
            Text(
