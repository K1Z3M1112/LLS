...
data class LsfgConfig(
    ...
    /**
     * sharp-bilinear-simple params (see gpu_postprocess.comp), the only
     * scaling technique the app uses:
     * https://gamingprojects.wordpress.com/2017/12/03/reducing-pixel-blur-and-distortion/
     * sharpness: 0 (plain bilinear) .. 1 (full integer-prescale snap, matches
     * the blog exactly). smoothness: 0 (linear blend at the pixel boundary,
     * matches the blog exactly) .. 1 (smoothstep-eased blend).
     */
    val gpuSharpness: Float,
    val gpuSmoothness: Float,
...
class LsfgPreferences(ctx: Context) {
    ...
    fun load(): LsfgConfig = LsfgConfig(
        ...
        gpuSharpness = prefs.getFloat(KEY_GPU_SHARPNESS, 1.0f).coerceIn(0.0f, 1.0f),
        gpuSmoothness = prefs.getFloat(KEY_GPU_SMOOTHNESS, 0.0f).coerceIn(0.0f, 1.0f),
    ...
    fun setGpuSharpness(value: Float) =
        prefs.edit().putFloat(KEY_GPU_SHARPNESS, value.coerceIn(0.0f, 1.0f)).apply()
    fun setGpuSmoothness(value: Float) =
        prefs.edit().putFloat(KEY_GPU_SMOOTHNESS, value.coerceIn(0.0f, 1.0f)).apply()
    ...
    companion object {
        ...
        private const val KEY_GPU_SMOOTHNESS = "gpu_smoothness"
