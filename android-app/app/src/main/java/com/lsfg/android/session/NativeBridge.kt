...
object NativeBridge {
     ...
     * plus [multiplier]-many output images, and creates a framegen context using the selected
     * [engine] (0 = Lossless.dll/LSFG_3_1(P), 1 = ncnn — see [com.lsfg.android.prefs.FrameGenEngine]).
     *
     * [ncnnParamPath]/[ncnnBinPath] are only read when engine == 1; pass empty strings for the
     * Lossless.dll engine. They must be real filesystem paths (not asset:// URIs) — extract the
     * bundled asset files to internal storage first, e.g. via [com.lsfg.android.session.NcnnModelAssets].
     *
    ...
    external fun initContext(
        ...
        queueDepth: Int,
        engine: Int,
        ncnnParamPath: String,
        ncnnBinPath: String,
        ncnnUseVulkanCompute: Boolean,
    ): Int
