...
object NativeBridge {
    ...
    // -------------------------------------------------------------------
    // RIFE backend (FrameGenBackend.RIFE_NCNN). See rife_ncnn.hpp for
    // status — this is a standalone CPU inference path, not yet hooked
    // into the per-frame render loop above. DllPickerScreen currently
    // keeps this backend locked in the UI until that wiring lands.
    // -------------------------------------------------------------------

    /**
     * Loads assets/models/rife/flownet.{param,bin} and registers the
     * custom rife.Warp layer the graph needs. [assetManager] should be
     * `context.assets` from Kotlin (mapped to android.content.res.AssetManager
     * on the JNI side via AAssetManager_fromJava).
     *
     * @return 0 on success, negative ncnn/asset error otherwise.
     */
    external fun initRifeEngine(assetManager: android.content.res.AssetManager): Int

    /**
     * Runs one interpolation between [rgba0Buffer] and [rgba1Buffer] at
     * [timestep] (0..1), writing the result into [outRgbaBuffer]. All
     * three must be `java.nio.ByteBuffer.allocateDirect(width * height * 4)`
     * — JNI reads/writes their native memory directly, no copy through the
     * JVM heap.
     *
     * @return 0 on success.
     */
    external fun rifeInterpolate(
        rgba0Buffer: java.nio.ByteBuffer,
        rgba1Buffer: java.nio.ByteBuffer,
        outRgbaBuffer: java.nio.ByteBuffer,
        width: Int,
        height: Int,
        timestep: Float,
    ): Int

    /** Releases the RIFE engine. Safe to call multiple times / when never loaded. */
    external fun destroyRifeEngine()
}
