...
/**
 * JNI bridge to liblsfg-android.so.
...
object NativeBridge {
    ...
    /** Returns 0 on success, negative on error (see kErr* constants). */
    ...
    // --- NCNN AI frame-gen backend ------------------------------------------
    /**
     * Load an NCNN model at runtime.
     * [paramPath] = absolute path to the .ncnn.param file;
     * the .ncnn.bin must be in the same directory with the same stem.
     *
     * Returns true on success. When NCNN support was not compiled in,
     * always returns false.
     */
    external fun ncnnLoadModel(paramPath: String): Boolean

    /** True if a model is currently loaded and ready for inference. */
    external fun ncnnIsLoaded(): Boolean

    /**
     * True if the loaded model is running on the GPU via ncnn's Vulkan
     * compute backend (false = CPU fallback).
     */
    external fun ncnnIsVulkan(): Boolean

    /** Release the loaded model and free GPU resources. */
    external fun ncnnUnload()

    /** True if the binary was compiled with NCNN support (-DLSFGVK_ENABLE_NCNN=ON). */
    external fun ncnnIsAvailable(): Boolean
