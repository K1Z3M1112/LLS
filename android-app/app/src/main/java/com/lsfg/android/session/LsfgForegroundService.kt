    ...
    private fun handleStart(intent: Intent) {
        ...
        ov.onSurfaceReady { surface, w, h ->
            ...
            if (!initialCaptureStarted) {
                ...
                // ncnn engine ships its own model — extract the bundled asset pair to a
                // real filesystem path once (idempotent) before handing it to native code.
                // Empty strings for the Lossless.dll engine; lsfg_jni.cpp only reads these
                // when engine == 1.
                val ncnnPaths = if (cfg.engine == FrameGenEngine.NCNN)
                    NcnnModelAssets.ensureExtracted(this, cfg.ncnnUseFp16)
                else null
                val rc = runCatching {
                    NativeBridge.initContext(
                        ...
                        engine = cfg.engine.nativeOrdinal,
                        ncnnParamPath = ncnnPaths?.paramPath ?: "",
                        ncnnBinPath = ncnnPaths?.binPath ?: "",
                        ncnnUseVulkanCompute = true,
                    )
                }.getOrElse { e ->
