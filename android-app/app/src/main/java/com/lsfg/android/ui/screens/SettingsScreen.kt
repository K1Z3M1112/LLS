    ...
    Column(
        ...
        GamingCard(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp)) {
            ...
            SettingRow("HDR Mode", "Pass through HDR metadata to display") {
                GamingSwitch(
                    checked = config.hdr,
                    onCheckedChange = { onConfigChange(config.copy(hdr = it)) },
                )
            }
        }
        ...
        GamingCard(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp)) {
            SettingRow("Show HUD Overlay", "Display live stats over your game") {
                GamingSwitch(
                    ...
                    onCheckedChange = { onConfigChange(config.copy(showHud = it)) },
                )
            }
            HorizontalDivider(color = SpaceBorder, thickness = 0.5.dp)
            SettingRow("Shizuku Timing", "Use Shizuku metrics side-channel for pacing") {
                GamingSwitch(
                    checked = config.shizukuTimingEnabled,
                    onCheckedChange = { onConfigChange(config.copy(shizukuTimingEnabled = it)) },
                )
            }
            HorizontalDivider(color = SpaceBorder, thickness = 0.5.dp)
...
private fun NpuSettingsPanel(config: LsfgConfig, onConfigChange: (LsfgConfig) -> Unit) {
    GamingCard(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp),
        ...
        SettingRow("Enable NPU Post-Processing", null) {
            GamingSwitch(
                checked = config.npuPostProcessingEnabled,
                onCheckedChange = { onConfigChange(config.copy(npuPostProcessingEnabled = it)) },
            )
        }
        HorizontalDivider(color = SpaceBorder, thickness = 0.5.dp)
        ...
        SettingRow("Upscale Factor",
            ...
            GamingSwitch(
                checked = config.npuUpscaleFactor == 2,
                onCheckedChange = { onConfigChange(config.copy(npuUpscaleFactor = if (it) 2 else 1)) },
            )
        }
        HorizontalDivider(color = SpaceBorder, thickness = 0.5.dp)
        SettingRow("FP16 (NPU)", "Use relaxed FP16 inference on NPU") {
            GamingSwitch(
                checked = config.npuFp16,
                onCheckedChange = { onConfigChange(config.copy(npuFp16 = it)) },
            )
        }
    }
}
...
private fun CpuSettingsPanel(config: LsfgConfig, onConfigChange: (LsfgConfig) -> Unit) {
    GamingCard(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp),
        ...
        SettingRow("Enable CPU Post-Processing", null) {
            GamingSwitch(
                checked = config.cpuPostProcessingEnabled,
                onCheckedChange = { onConfigChange(config.copy(cpuPostProcessingEnabled = it)) },
            )
        }
        HorizontalDivider(color = SpaceBorder, thickness = 0.5.dp)
...
private fun GpuSettingsPanel(config: LsfgConfig, onConfigChange: (LsfgConfig) -> Unit) {
    GamingCard(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp),
        ...
        SettingRow("Enable GPU Post-Processing", null) {
            GamingSwitch(
                checked = config.gpuPostProcessingEnabled,
                onCheckedChange = { onConfigChange(config.copy(gpuPostProcessingEnabled = it)) },
            )
        }
        HorizontalDivider(color = SpaceBorder, thickness = 0.5.dp)
        SettingRow("Stage", if (config.gpuStage == 0) "Before LSFG" else "After LSFG") {
            GamingSwitch(
                checked = config.gpuStage == 1,
                onCheckedChange = { onConfigChange(config.copy(gpuStage = if (it) 1 else 0)) },
            )
        }
        HorizontalDivider(color = SpaceBorder, thickness = 0.5.dp)
