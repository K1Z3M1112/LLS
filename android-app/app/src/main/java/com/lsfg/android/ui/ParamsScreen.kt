...
fun ParamsFrameGenPacingScreen(nav: NavHostController) {
    ...
    Column(
        ...
        // ---- Frame gen engine -------------------------------------------------------
        // Choosing the engine here drives which of the settings below apply — see the
        // `state.engine == ...` guards further down this screen. Both engines share the
        // multiplier/flow-scale/HDR/anti-artifacts settings; engine-specific settings
        // (FP16 SPIR-V for Lossless.dll, FP16 model weights for ncnn) are hidden when
        // they don't apply to the selected engine, per the native RenderLoopConfig
        // contract in lsfg_render_loop.hpp.
        LsfgCard {
            SectionHeader(eyebrow = stringResource(R.string.section_frame_gen_engine), title = null)
            Spacer(Modifier.height(4.dp))
            Text(
                text = stringResource(R.string.frame_gen_engine_desc),
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                FilterChip(
                    selected = state.engine == FrameGenEngine.LOSSLESS_DLL,
                    onClick = {
                        prefs.setEngine(FrameGenEngine.LOSSLESS_DLL)
                        refreshConfigState(prefs)
                    },
                    label = { Text(stringResource(R.string.frame_gen_engine_lossless_dll)) },
                    modifier = Modifier.weight(1f),
                )
                FilterChip(
                    selected = state.engine == FrameGenEngine.NCNN,
                    onClick = {
                        prefs.setEngine(FrameGenEngine.NCNN)
                        refreshConfigState(prefs)
                    },
                    label = { Text(stringResource(R.string.frame_gen_engine_ncnn)) },
                    modifier = Modifier.weight(1f),
                )
            }
        }
        ...
        LsfgCard {
            ...
            Spacer(Modifier.height(8.dp))
            HorizontalDivider(color = MaterialTheme.colorScheme.outlineVariant)
            if (state.engine == FrameGenEngine.LOSSLESS_DLL) {
                ToggleRow(
            ...
            if (fp16Available && state.engine == FrameGenEngine.LOSSLESS_DLL) {
                HorizontalDivider(color = MaterialTheme.colorScheme.outlineVariant)
                ToggleRow(
            ...
            if (state.engine == FrameGenEngine.NCNN) {
                HorizontalDivider(color = MaterialTheme.colorScheme.outlineVariant)
                ToggleRow(
                    icon = Icons.Filled.Memory,
                    title = stringResource(R.string.param_ncnn_fp16),
                    description = stringResource(R.string.param_ncnn_fp16_desc),
                    checked = state.ncnnUseFp16,
                    onCheckedChange = {
