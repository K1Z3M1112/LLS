...
import com.lsfg.android.prefs.FrameGenBackend
...
private fun BackendOptionRow(
    ...
    Card(
        ...
        border = BorderStroke(
            ...
            color = if (selected) LsfgPrimary else MaterialTheme.colorScheme.outlineVariant,
        ),
        ...
        Row(
            ...
            RadioButton(
                ...
                enabled = enabled,
                colors = RadioButtonDefaults.colors(selectedColor = LsfgPrimary),
            )
...
fun DllPickerScreen(nav: NavHostController) {
    ...
    Column(
        ...
        LsfgCard {
            ...
            BackendOptionRow(
                title = "Lossless.dll (DXBC)",
                subtitle = "Requires your own Lossless.dll from Steam. Fully working today.",
                selected = state.frameGenBackend == FrameGenBackend.LOSSLESS_DXBC,
                enabled = true,
                onClick = {
                    prefs.setFrameGenBackend(FrameGenBackend.LOSSLESS_DXBC)
                    refreshConfigState(prefs)
                },
            )
            Spacer(Modifier.height(8.dp))
            BackendOptionRow(
                title = "RIFE (ncnn-vulkan)",
                subtitle = "flownet model bundled, CPU inference works standalone. " +
                    "Not wired into the live render loop yet — coming soon.",
                selected = state.frameGenBackend == FrameGenBackend.RIFE_NCNN,
                enabled = false,
                onClick = {
                    prefs.setFrameGenBackend(FrameGenBackend.RIFE_NCNN)
                    refreshConfigState(prefs)
                },
            )
        }

        if (state.frameGenBackend == FrameGenBackend.RIFE_NCNN) {
            // Backend not implemented natively yet — nothing else on this screen applies.
            return@Column
        }
