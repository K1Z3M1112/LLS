...
// AI engine mode
enum class AiEngineMode(val value: String) {
    LOSSLESS_DLL("lossless_dll"),   // default: use extracted Vulkan shaders
    NCNN_AI("ncnn_ai");             // use bundled/custom NCNN model

    companion object {
        fun from(v: String?) = entries.firstOrNull { it.value == v } ?: LOSSLESS_DLL
    }
}

// Which NCNN precision variant to load
enum class NcnnPrecision(val value: String, val label: String) {
    FP32("fp32", "FP32"),
    FP16("fp16", "FP16");

    companion object {
        fun from(v: String?) = entries.firstOrNull { it.value == v } ?: FP32
    }
}
...
data class LsfgConfig(
    ...
    // ── AI Engine (NCNN) ──────────────────────────────────────────────────
    /** Whether to use the NCNN AI model instead of the Lossless.dll Vulkan pipeline. */
    val aiEngineMode: AiEngineMode = AiEngineMode.LOSSLESS_DLL,
    /** FP32 or FP16 precision for the NCNN model. */
    val ncnnPrecision: NcnnPrecision = NcnnPrecision.FP32,
    /**
     * Whether to use a custom user-supplied model file instead of the
     * bundled model. When false the app copies from assets on first launch.
     */
    val ncnnUseCustomModel: Boolean = false,
    /**
     * Internal-storage absolute path to the custom .param file chosen by
     * the user. Only meaningful when [ncnnUseCustomModel] is true.
     */
    val ncnnCustomParamPath: String = "",
    /**
     * Display name for the custom model (filename shown in UI).
     */
    val ncnnCustomModelName: String = "",
)
...
class LsfgPreferences(ctx: Context) {
    ...
    fun load(): LsfgConfig = LsfgConfig(
        ...
        // NCNN
        aiEngineMode            = AiEngineMode.from(prefs.getString("ai_engine_mode", null)),
        ncnnPrecision           = NcnnPrecision.from(prefs.getString("ncnn_precision", null)),
        ncnnUseCustomModel      = prefs.getBoolean("ncnn_use_custom", false),
        ncnnCustomParamPath     = prefs.getString("ncnn_custom_param", "") ?: "",
        ncnnCustomModelName     = prefs.getString("ncnn_custom_name", "") ?: "",
    )
    ...
    fun save(c: LsfgConfig) = prefs.edit().apply {
        ...
        putString("ai_engine_mode", c.aiEngineMode.value)
        putString("ncnn_precision", c.ncnnPrecision.value)
        putBoolean("ncnn_use_custom", c.ncnnUseCustomModel)
        putString("ncnn_custom_param", c.ncnnCustomParamPath)
        putString("ncnn_custom_name", c.ncnnCustomModelName)
    }.apply()
    ...
    fun setNpuPostProcessingPreset(v: NpuPostProcessingPreset) =
        prefs.edit().putInt("npu_preset", v.nativeValue).apply()
    fun setCpuPostProcessingEnabled(v: Boolean) = prefs.edit().putBoolean("cpu_enabled", v).apply()
    fun setCpuPostProcessingPreset(v: CpuPostProcessingPreset) =
        prefs.edit().putInt("cpu_preset", v.nativeValue).apply()
    fun setGpuPostProcessingEnabled(v: Boolean) = prefs.edit().putBoolean("gpu_enabled", v).apply()
    fun setVsyncRefreshOverride(v: VsyncRefreshOverride) =
        prefs.edit().putInt("vsync_override", v.ordinal).apply()
    fun setDllPath(path: String, sha256: String) {
        prefs.edit().putString("dll_path", path).putString("dll_sha256", sha256).apply()
    }

    // NCNN setters
    fun setAiEngineMode(v: AiEngineMode) =
        prefs.edit().putString("ai_engine_mode", v.value).apply()
    fun setNcnnPrecision(v: NcnnPrecision) =
        prefs.edit().putString("ncnn_precision", v.value).apply()
    fun setNcnnUseCustomModel(v: Boolean) =
        prefs.edit().putBoolean("ncnn_use_custom", v).apply()
    fun setNcnnCustomModel(paramPath: String, displayName: String) = prefs.edit()
        .putBoolean("ncnn_use_custom", true)
        .putString("ncnn_custom_param", paramPath)
        .putString("ncnn_custom_name", displayName)
        .apply()
    fun clearNcnnCustomModel() = prefs.edit()
        .putBoolean("ncnn_use_custom", false)
        .putString("ncnn_custom_param", "")
        .putString("ncnn_custom_name", "")
        .apply()
}
