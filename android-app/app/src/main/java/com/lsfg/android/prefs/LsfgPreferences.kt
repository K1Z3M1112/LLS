...
data class LsfgConfig(
     ...
     * 50 is neutral (matches the original fixed-threshold behaviour).
     * Higher suppresses generated frames more readily (fewer visible
     * artifacts, more drops to the real capture rate); lower suppresses
     * less (smoother frame count, more risk of visible optical-flow
     * artifacts on fast motion). Only takes effect while [antiArtifacts]
     * is enabled.
     */
    val antiArtifactsIntensity: Int,
    /**
...
class LsfgPreferences(ctx: Context) {
    ...
    fun load(): LsfgConfig = LsfgConfig(
        ...
        antiArtifactsIntensity = prefs.getInt(KEY_ANTI_ARTIFACTS_INTENSITY, 50).coerceIn(1, 100),
    ...
    fun setAntiArtifactsIntensity(value: Int) =
        prefs.edit().putInt(KEY_ANTI_ARTIFACTS_INTENSITY, value.coerceIn(1, 100)).apply()
    ...
    companion object {
        ...
        private const val KEY_ANTI_ARTIFACTS_INTENSITY = "anti_artifacts_intensity"
