...
/**
 * Which frame-generation engine the render loop should use.
 *
 *  - [LOSSLESS_DXBC] (default, only fully implemented path today): extracts DXBC shaders
 *    from the user-supplied Lossless.dll and translates them to SPIR-V via `dxbc-to-spirv`.
 *    See [ShaderExtractor] / `lsfg_render_loop.cpp`.
 *  - [RIFE_NCNN]: placeholder for a motion-vector-free optical-flow interpolation backend
 *    (e.g. RIFE via ncnn-vulkan) that would not require Lossless.dll at all. No native
 *    implementation exists yet — `lsfg_jni.cpp`/`lsfg_render_loop.cpp` only know how to run
 *    the DXBC-derived shaders. Selecting this in the UI is intentionally gated off
 *    (see DllPickerScreen) until that engine is actually wired up, so the app never
 *    silently no-ops or crashes pretending to run a backend that isn't there.
 */
enum class FrameGenBackend(val prefValue: String) {
    LOSSLESS_DXBC("lossless_dxbc"),
    RIFE_NCNN("rife_ncnn");

    companion object {
        fun fromPref(value: String?): FrameGenBackend =
            values().firstOrNull { it.prefValue == value } ?: LOSSLESS_DXBC
    }
}
