...
data class LsfgConfig(
    ...
    /**
     * Which frame-generation backend to run: the reverse-engineered
     * Lossless.dll pipeline (requires [dllUri]/[shadersReady]) or the
     * bundled ncnn model (requires no user file — see [NcnnModelAssets]).
     * Settings below that only apply to one engine (e.g. [framegenFp16] is
     * Lossless.dll-only, [ncnnUseFp16] is ncnn-only) are simply ignored by
     * the native side when the other engine is active.
     */
    val engine: FrameGenEngine,
    val multiplier: Int,
    val flowScale: Float,
    ...
    /**
     * ncnn engine only: selects the fp16 vs fp32 weight variant of the
     * bundled frame_interpolator_v2 model (see NcnnModelAssets). Smaller/
     * faster at fp16, more precise at fp32. Ignored by the native side when
     * [engine] is LOSSLESS_DLL.
     */
    val ncnnUseFp16: Boolean,
...
/**
 * Which frame-generation backend NativeBridge.initContext should run.
 * Mirrors the native `FrameGenEngine` enum in lsfg_render_loop.hpp — the
 * ordinal sent over JNI (0/1) must stay in sync with that C++ enum.
 */
enum class FrameGenEngine(val prefValue: String, val nativeOrdinal: Int) {
    LOSSLESS_DLL("lossless_dll", 0),
    NCNN("ncnn", 1);

    companion object {
        fun fromPref(value: String?): FrameGenEngine =
            values().firstOrNull { it.prefValue == value } ?: LOSSLESS_DLL
    }
}
...
class LsfgPreferences(ctx: Context) {
    ...
    fun load(): LsfgConfig = LsfgConfig(
        ...
        engine = FrameGenEngine.fromPref(prefs.getString(KEY_ENGINE, null)),
        ...
        ncnnUseFp16 = prefs.getBoolean(KEY_NCNN_FP16, false),
    ...
    fun setEngine(value: FrameGenEngine) = prefs.edit().putString(KEY_ENGINE, value.prefValue).apply()
    fun setNcnnUseFp16(value: Boolean) = prefs.edit().putBoolean(KEY_NCNN_FP16, value).apply()
    ...
    companion object {
        ...
        private const val KEY_ENGINE = "framegen_engine"
        private const val KEY_NCNN_FP16 = "ncnn_use_fp16"
