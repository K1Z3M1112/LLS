        ...
        externalNativeBuild {
            cmake {
                ...
                arguments += listOf(
                    ...
                    // The NDK's CMake toolchain disables RTTI and exceptions by
                    // default unless explicitly requested here. android_shader_loader.cpp
                    // and lsfg_render_loop.cpp rely on try/catch, so both must be on.
                    "-DANDROID_CPP_FEATURES=rtti exceptions",
                    "-DCMAKE_ANDROID_RTTI=ON",
                    "-DCMAKE_ANDROID_EXCEPTIONS=ON",
                    "-DCMAKE_SHARED_LINKER_FLAGS=-Wl,--gc-sections,--icf=safe"
                )
                // NCNN AI frame-gen backend: opt-in via the NCNN_ANDROID_ROOT env var
                // (set by CI after building ncnn per-ABI -- see
                // .github/workflows/build-apk.yml). Left unset, LSFGVK_ENABLE_NCNN
                // stays OFF and the app builds exactly as before (Lossless.dll path
                // only) -- this keeps local builds working without ncnn installed.
                val ncnnAndroidRoot = System.getenv("NCNN_ANDROID_ROOT")
                if (ncnnAndroidRoot != null) {
                    arguments += listOf(
                        "-DLSFGVK_ENABLE_NCNN=ON",
                        "-DNCNN_ANDROID_ROOT=$ncnnAndroidRoot"
                    )
                }
            }
        }
