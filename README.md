...
- **ncnn** — an alternative engine backed by the bundled
  ...
  No DLL required; just pick it from the engine selector. Built against the
  official prebuilt ncnn Android+Vulkan package (NOT built from source) —
  CI downloads it automatically (see `.github/workflows/build-apk.yml`).
  Building locally: download the matching release zip from
  https://github.com/Tencent/ncnn/releases (the `*-android-vulkan.zip` asset;
  check `NCNN_VERSION` in the workflow file for the exact version this repo
  targets), extract it, and export `NCNN_ANDROID_ROOT` to the extracted
  folder (the one containing `arm64-v8a/`, `x86_64/`, ...) before building:
  ```bash
