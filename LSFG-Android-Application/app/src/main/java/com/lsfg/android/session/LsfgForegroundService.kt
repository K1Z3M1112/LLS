...
class LsfgForegroundService : Service() {
    ...
    private var aiLiteEngine: TfliteFrameGenEngine? = null
    private var aiLitePresenter: AiLitePresenter? = null
    ...
    override fun onDestroy() {
        ...
        capture?.stop()
        capture = null
        aiLitePresenter?.detach()
        aiLitePresenter = null
        aiLiteEngine?.close()
        aiLiteEngine = null
        shizukuCapture?.stop()
        shizukuCapture = null
    ...
    /**
     * AI_LITE branch of session startup. Deliberately independent of the native
     * LSFG context (NativeBridge.initContext/destroyContext, setOutputSurface):
     * this path never touches it, so it can't destabilize the Vulkan pipeline.
     * Frames still flow through the existing ImageReader/VirtualDisplay set up
     * by cap.setLsfgMode() a few lines above the call site — see
     * [CaptureEngine.aiLiteModeEnabled], which redirects them here as
     * software Bitmaps instead of native HardwareBuffer pushes.
     */
    private fun startAiLiteSession(w: Int, h: Int, surface: Surface, cfg: com.lsfg.android.prefs.LsfgConfig) {
        val cap = capture ?: return
        val modelFile = File(TfliteFrameGenEngine.modelDir(this), "framegen.tflite")
        if (!modelFile.exists()) {
            LsfgLog.w(TAG, "AI-lite selected but framegen.tflite missing; falling back to mirror")
            cap.setLsfgNativeInputEnabled(false)
            cap.aiLiteModeEnabled = false
            cap.setSurface(surface, w, h)
            overlay?.updateStatus("AI-lite: model missing, mirroring raw capture")
            return
        }

        val engine = TfliteFrameGenEngine(this, cpuThreads = cfg.tfliteCpuThreads)
        val loaded = engine.load(modelFile)
        if (!loaded) {
            LsfgLog.w(TAG, "AI-lite model failed to load at session start; falling back to mirror")
            engine.close()
            cap.setLsfgNativeInputEnabled(false)
            cap.aiLiteModeEnabled = false
            cap.setSurface(surface, w, h)
            overlay?.updateStatus("AI-lite: model load failed, mirroring raw capture")
            return
        }

        val presenter = AiLitePresenter(engine, cfg.multiplier.coerceIn(2, 4))
        presenter.attach(surface)
        cap.setAiLiteFrameListener { bitmap, ts -> presenter.submitFrame(bitmap, ts) }
        cap.setLsfgNativeInputEnabled(false)
        cap.aiLiteModeEnabled = true

        aiLiteEngine = engine
        aiLitePresenter = presenter
        lsfgContextActive = false // native context was never created on this path
        activeRenderW = w
        activeRenderH = h

        val accel = if (engine.usingNnapiAccelerator) "NPU" else "CPU"
        overlay?.updateStatus("AI-lite: active ${w}×${h} ×${cfg.multiplier} ($accel)")
        maybeStartBenchmark()
    }
    ...
    private fun handleStart(intent: Intent) {
        ...
        ov.onSurfaceReady { surface, w, h ->
            ...
            if (!initialCaptureStarted) {
                ...
                if (cfg.framegenEngine == FramegenEngine.AI_LITE && cfg.tfliteModelReady) {
                    LsfgLog.i(TAG, "Starting AI-lite session ${w}x${h} x${cfg.multiplier} (native LSFG context skipped)")
                    startAiLiteSession(w, h, surface, cfg)
                    return@onSurfaceReady
                }
    ...
    private fun reinitLsfgContext(width: Int = lastSurfaceW, height: Int = lastSurfaceH) {
        ...
        if (aiLitePresenter != null) {
            // AI-lite sessions never created a native context, so there's nothing to
            // tear down/recreate here. Multiplier/flow-scale live-edit for AI-lite
            // is a follow-up — for now just log so it's obvious why nothing happened.
            LsfgLog.i(TAG, "reinitLsfgContext skipped — AI-lite session active, no native context to reinit")
            return
        }
        if (width == 0 || height == 0) {
