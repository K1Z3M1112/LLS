...
data class LsfgConfig(
    ...
    /**
     * Frame-generation backend. LSFG_SHADER is the existing Vulkan pipeline
     * (framegen/ submodule, requires the user's Lossless.dll). AI_LITE runs
     * a small on-device interpolation network (e.g. a RIFE-lite export)
     * through LiteRT/TensorFlow Lite with the NNAPI delegate — see
     * [TfliteFrameGenEngine]. AI_LITE trades quality for a much smaller
     * memory/compute footprint and doesn't need the DLL at all.
     */
    val framegenEngine: FramegenEngine,
    val tfliteModelUri: String?,
    val tfliteModelDisplayName: String?,
    val tfliteModelReady: Boolean,
    /** Caps CPU threads used by the TFLite interpreter when NNAPI/GPU delegates aren't available. */
    val tfliteCpuThreads: Int,
)

/**
 * Which engine produces the interpolated frames.
 *  - [LSFG_SHADER]: existing Vulkan shader pipeline extracted from the
 *    user's Lossless.dll. Highest quality, GPU-bound, heavier RAM use.
 *  - [AI_LITE]: small NNAPI/TFLite network (e.g. RIFE-lite, a few MB /
 *    a few million params). Lower quality but a much smaller memory
 *    footprint — good for low-RAM devices or when no DLL is available.
 */
enum class FramegenEngine(val prefValue: String) {
    LSFG_SHADER("lsfg_shader"),
    AI_LITE("ai_lite");

    companion object {
        fun fromPref(value: String?): FramegenEngine =
            values().firstOrNull { it.prefValue == value } ?: LSFG_SHADER
    }
}
...
class LsfgPreferences(ctx: Context) {
    ...
    fun load(): LsfgConfig = LsfgConfig(
        ...
        framegenEngine = FramegenEngine.fromPref(prefs.getString(KEY_FRAMEGEN_ENGINE, null)),
        tfliteModelUri = prefs.getString(KEY_TFLITE_URI, null),
        tfliteModelDisplayName = prefs.getString(KEY_TFLITE_NAME, null),
        tfliteModelReady = prefs.getBoolean(KEY_TFLITE_READY, false),
        tfliteCpuThreads = prefs.getInt(KEY_TFLITE_THREADS, 2).coerceIn(1, 4),
    )
    ...
    fun setFramegenEngine(value: FramegenEngine) = prefs.edit()
        .putString(KEY_FRAMEGEN_ENGINE, value.prefValue)
        .apply()
    fun setTfliteModel(uri: String, displayName: String) = prefs.edit()
        .putString(KEY_TFLITE_URI, uri)
        .putString(KEY_TFLITE_NAME, displayName)
        .putBoolean(KEY_TFLITE_READY, false)
        .apply()
    fun setTfliteModelReady(ready: Boolean) = prefs.edit().putBoolean(KEY_TFLITE_READY, ready).apply()
    fun setTfliteCpuThreads(value: Int) = prefs.edit()
        .putInt(KEY_TFLITE_THREADS, value.coerceIn(1, 4))
        .apply()
    ...
    companion object {
        ...
        private const val KEY_FRAMEGEN_ENGINE = "framegen_engine"
        private const val KEY_TFLITE_URI = "tflite_model_uri"
        private const val KEY_TFLITE_NAME = "tflite_model_name"
        private const val KEY_TFLITE_READY = "tflite_model_ready"
        private const val KEY_TFLITE_THREADS = "tflite_cpu_threads"
