...
dependencies {
    ...
    // AI-lite frame-generation engine (TfliteFrameGenEngine.kt): small on-device
    // interpolation model + NNAPI delegate, as an alternative to the LSFG Vulkan
    // shader pipeline. tensorflow-lite-select-tf-ops is deliberately NOT included
    // to keep the APK/runtime footprint small; models must use core TFLite ops.
    // Google's current guidance is to migrate to `com.google.ai.edge.litert:litert`
    // (the renamed successor); swap these two lines for that when you're ready.
