...
namespace Mini {
    ...
    public:
        ...
        ///
        /// Create a plain, device-local image with no external memory export
        /// (no fd, no AHB). Used by the NCNN backend path, which stays on a
        /// single Vulkan device and never needs to share this image's memory
        /// with anything else -- this avoids the OPAQUE_FD-on-Android
        /// portability issues the fd-based constructor above works around.
        ///
        /// @throws LSFG::vulkan_error if object creation fails.
        ///
        Image(VkDevice device, VkPhysicalDevice physicalDevice, VkExtent2D extent, VkFormat format,
            VkImageUsageFlags usage, VkImageAspectFlags aspectFlags, bool plainNoExport);
