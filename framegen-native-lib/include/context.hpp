...
private:
    ///
    /// Runs frame generation via the NCNN backend instead of lsfg: copies
    /// frame_0/frame_1 to host memory, runs inference, and uploads the
    /// generated frames into out_n. Called from present() in place of the
    /// LSFG_3_1::presentContext()/waitIdle() calls when Config::activeConf.useNcnn
    /// is set. Fully synchronous (like the Android waitIdle() path), since
    /// there's no second Vulkan device to synchronize against.
    ///
    /// @throws LSFG::vulkan_error if any Vulkan call fails.
    /// @throws std::runtime_error if NCNN inference fails.
    ///
    VkResult presentNcnn(const Hooks::DeviceInfo& info, const void* pNext, VkQueue queue,
        const std::vector<VkSemaphore>& gameRenderSemaphores, uint32_t presentIdx);
...
private:
    ...
    // NCNN backend path (alternative to the lsfg/Lossless.dll pipeline).
    // When active, frame_0/frame_1/out_n above are still used as the GPU-side
    // handoff points, but instead of lsfg's own Vulkan device generating into
    // out_n, we read frame_0/frame_1 back to the host, run inference there,
    // and upload the results into out_n ourselves.
    bool ncnnMode{false};
    std::shared_ptr<Inference::NcnnBackend> ncnnBackend;
    Mini::Buffer stagingIn0, stagingIn1;     // host-visible copies of frame_0/frame_1
    std::vector<Mini::Buffer> stagingOut;    // host-visible staging for each out_n
