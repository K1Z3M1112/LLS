...
namespace Config {
    ...
    struct Configuration {
        ...
        /// Path to an NCNN model's .param file (matching .bin must sit next to it).
        /// Ignored unless `useNcnn` is true.
        std::string ncnnModel;
        /// Whether to use the NCNN model above (Vulkan mobile GPU backend) instead
        /// of the Lossless.dll-extracted Vulkan pipeline.
        bool useNcnn{false};
