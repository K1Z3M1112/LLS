...
pub fn register_signals(sidebar_: pane::PaneSidebar, main: &pane::PaneMain) {
    ...
    // NCNN model path — text entry, same behavior as dll
    let ncnn_model = main.ncnn_model.imp();
    ncnn_model.entry.connect_changed(|entry| {
        let _ = config::edit_config(|config| {
            let mut text = entry.text().to_string();
            if text.trim().is_empty() {
                config.global.ncnn_model = None;
            } else {
                if text.contains("~") {
                    let home = std::env::var("HOME").unwrap_or_else(|_| String::from("/"));
                    text = text.replace("~", &home);
                }
                config.global.ncnn_model = Some(text);
            }
        });
    });

    // NCNN model path — file picker button, same behavior as dll
    let entry = ncnn_model.entry.clone();
    ncnn_model.btn.connect_clicked(move |btn| {
        let dialog = gtk::FileDialog::new();
        dialog.set_title("Select NCNN Model (.param)");

        let filter = gtk::FileFilter::new();
        filter.set_name(Some("NCNN model (.param)"));
        filter.add_pattern("*.param");
        ...
        dialog.open(Some(&window), gio::Cancellable::NONE, move |result| {
            ...
            entry.set_text(&path_str);
            let _ = config::edit_config(|config| {
                config.global.ncnn_model = Some(path_str);
            });
        });
    });

    // Use NCNN backend — toggle switch
    let use_ncnn = main.use_ncnn.imp();
    use_ncnn.switch.connect_state_notify(|switch| {
        let _ = config::edit_config(|config| {
            config.global.use_ncnn = switch.state();
        });
    });
