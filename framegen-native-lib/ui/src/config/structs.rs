...
pub struct TomlGlobal {
    ...
    /// Path to a custom NCNN model (.param file; matching .bin must sit alongside it)
    #[serde(default)]
    pub ncnn_model: Option<String>,
    /// Use the NCNN (Vulkan mobile GPU) backend instead of Lossless.dll
    #[serde(default)]
    pub use_ncnn: bool
}
