...
namespace {
    ...
    // NCNN host-readback staging path
...
namespace {
    ...
    VkResult layer_vkCreateDevice( // NOLINTBEGIN
        ...
        try {
            ...
            // NCNN host-readback staging path (best-effort: only needed if use_ncnn is set,
            // so we don't hard-fail `success` on these -- just warn if missing)
            if (!initDeviceFunc(*pDevice, "vkCreateBuffer", &next_vkCreateBuffer))
                std::cerr << "(NCNN backend will be unavailable: missing vkCreateBuffer)\n";
            initDeviceFunc(*pDevice, "vkDestroyBuffer", &next_vkDestroyBuffer);
            initDeviceFunc(*pDevice, "vkGetBufferMemoryRequirements", &next_vkGetBufferMemoryRequirements);
            initDeviceFunc(*pDevice, "vkBindBufferMemory", &next_vkBindBufferMemory);
            initDeviceFunc(*pDevice, "vkMapMemory", &next_vkMapMemory);
            initDeviceFunc(*pDevice, "vkUnmapMemory", &next_vkUnmapMemory);
            initDeviceFunc(*pDevice, "vkCmdCopyImageToBuffer", &next_vkCmdCopyImageToBuffer);
            initDeviceFunc(*pDevice, "vkCmdCopyBufferToImage", &next_vkCmdCopyBufferToImage);
