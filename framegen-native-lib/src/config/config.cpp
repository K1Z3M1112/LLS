...
void Config::updateConfig(const std::string& file) {
    ...
    const Configuration global{
        ...
        .ncnnModel = toml::find_or(globalTable, "ncnn_model", std::string()),
        .useNcnn = toml::find_or(globalTable, "use_ncnn", false),
        .config_file = file,
    ...
    if (global.useNcnn && global.ncnnModel.empty())
        throw std::runtime_error("use_ncnn is enabled but ncnn_model path is empty");
    ...
    for (const auto& gameTable : gamesList.as_array()) {
        ...
        Configuration game{
            .enable = true,
            .dll = global.dll,
            .ncnnModel = toml::find_or(gameTable, "ncnn_model", global.ncnnModel),
            .useNcnn = toml::find_or(gameTable, "use_ncnn", global.useNcnn),
        ...
        if (game.flowScale < 0.25F || game.flowScale > 1.0F)
            throw std::runtime_error("Flow scale must be between 0.25 and 1.0");
        if (game.useNcnn && game.ncnnModel.empty())
            throw std::runtime_error("use_ncnn is enabled but ncnn_model path is empty for " + exe);
